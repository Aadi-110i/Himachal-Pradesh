...
import { OrbitControls, useTexture, Float, Text, Center, Environment, Stars } from '@react-three/drei';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import MainLayout from '../components/layout/MainLayout';
import { Sparkles, ArrowRight, X, Info, Landmark, MapPin, Compass } from 'lucide-react';
import { getRandomLocation } from '../data/locations';
import mapTexture from '../assets/map2.png';

function Globe({ isSpinning, onStop, texture }) {
  const globeRef = useRef();
  const rotationSpeed = useRef(0.005);

  useFrame((state, delta) => {
    if (isSpinning) {
      rotationSpeed.current = Math.min(rotationSpeed.current + 0.01, 0.2);
    } else {
...
const DiscoveryGlobe = () => {
  ...
  const handleSpin = () => {
    ...
    // Spin for 2 seconds then stop and reveal
    setTimeout(() => {
      setIsSpinning(false);
    ...
    <MainLayout>
      <div className="h-[calc(100vh-64px-56px)] bg-[#0a0a0b] relative overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="absolute top-12 left-0 right-0 z-10 text-center pointer-events-none">
           ...
           <h2 className="text-4xl md:text-6xl font-serif text-white mb-4">Spin for a <span className="italic opacity-40">Destination.</span></h2>
           <p className="text-sm text-white/30 uppercase tracking-widest font-bold">Click the globe to let fate decide your next pilgrimage.</p>
        </div>

        {/* 3D Scene */}
        <div className="w-full h-full cursor-pointer active:scale-95 transition-transform" onClick={handleSpin}>
          <Canvas shadows camera={{ position: [0, 0, 8], fov: 45 }}>
            ...
            <Suspense fallback={null}>
              <Float speed={1.5} rotationIntensity={0.5} floatIntensity={0.5}>
                <Globe isSpinning={isSpinning} texture={texture} />
              </Float>
        ...
        <AnimatePresence>
          {showReveal && selectedLoc && (
            <div className="absolute inset-0 z-20 flex items-center justify-center p-6 bg-black/40 backdrop-blur-md">
              <motion.div 
                initial={{ opacity: 0, scale: 0.8, y: 40 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.8, y: 40 }}
                className="w-full max-w-4xl bg-white rounded-[4rem] shadow-2xl overflow-hidden flex flex-col md:flex-row"
              >
                {/* Left: Image/Preview */}
          ...
          >
             <div className="flex flex-col items-center gap-4">
                <div className="bg-white/10 backdrop-blur-md px-6 py-3 rounded-full border border-white/10 flex items-center gap-3 text-white/60">
                   ...
                   <span className="text-[10px] font-bold uppercase tracking-widest">Click to Invoke Random Fate</span>
                </div>
             </div>
          </motion.div>
        )}
